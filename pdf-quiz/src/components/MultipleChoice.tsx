...
class Joke extends React.Component<Props, State> {
    state: State = {
    ...
    gradeQuiz = async () => {
        ...
        console.log(correctAnswers + output);
        const fetchData = await fetch('http://localhost:3001/processQuiz',
            {
                method: "POST",
                body: JSON.stringify({
    ...
    render() {
        return (
            <nav>
                <h1 className="h1">Joke Quiz on <b>{this.props.title.substring(0, this.props.title.length - 4)}</b></h1>
